...
async def upload_content(
    ...
    try:
        ...
        return JSONResponse(
            status_code=200,
            content={"message": "Content uploaded successfully", "content": content_item}
        )
        ...
        if "NoSuchBucket" in error_message:
            error_message = f"S3 bucket does not exist. Please check your infrastructure setup."
        elif "AccessDenied" in error_message:
            error_message = f"Access denied to S3 bucket. Please check your AWS credentials and permissions."
            ...
            error_message = "AWS signature verification failed. Please check your AWS secret key."
        
        raise HTTPException(status_code=500, detail=f"Error uploading content: {error_message}")
...
async def get_content_by_id(
    ...
    presigned_url = s3_service.generate_presigned_url(
        ...
        bucket_name=settings.S3_DOCUMENT_BUCKET if content["file_type"] != "audio" else settings.S3_AUDIO_BUCKET
    )
