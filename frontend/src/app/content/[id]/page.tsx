...
const ContentDetailPage = ({ params }: ContentDetailPageProps) => {
  ...
  useEffect(() => {
    ...
    initialize();
  }, [checkAuth, router, contentId, fetchContentDetails, fetchTranscription, fetchGeneratedContents]);
          ...
          <>
            ...
            <div className="bg-white shadow rounded-lg overflow-hidden">
              ...
              <div className="p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {content.content_type === 'audio' && !currentTranscription && (
                          <button
                        ...
                        >
                          {generationLoading && generationType === 'summary' ? 'Generating...' : 'Generate Summary'}
                        </button>
                        ...
                        >
                          {generationLoading && generationType === 'flashcards' ? 'Generating...' : 'Generate Flashcards'}
                        </button>
                        ...
                        >
                          {generationLoading && generationType === 'quiz' ? 'Generating...' : 'Generate Quiz'}
                        </button>
                        ...
                        >
                          {generationLoading && generationType === 'mindmap' ? 'Generating...' : 'Generate Mind Map'}
                        </button>
                      </div>
                    </div>
                  </div>
                ...
                {/* Transcription Tab */}
                {activeTab === 'transcription' && content.content_type === 'audio' && (
                  <div className="space-y-4">
                    {transcriptionError && (
                      <div className="bg-red-50 p-4 rounded-md">
                        ...
                        {currentTranscription.status === 'completed' ? (
                          <div className="mt-4 p-4 bg-gray-50 rounded-md">
                            {currentTranscription.segments ? (
                              <div className="space-y-2">
                                {currentTranscription.segments.map((segment: any, index: number) => (
                              ...
                              <p className="text-gray-700 whitespace-pre-line">{currentTranscription.text}</p>
                            )}
                          </div>
                        ) : currentTranscription.status === 'failed' ? (
                          <div className="mt-4 bg-red-50 p-4 rounded-md">
                            <p className="text-red-700">Transcription failed: {currentTranscription.error}</p>
                          </div>
                        ) : (
                          <div className="mt-4 text-center py-8">
                            <div className="flex flex-col items-center">
                              ...
                              <p className="mt-4 text-gray-500">Transcription in progress...</p>
                            </div>
                          </div>
                        )}
                      </div>
                ...
                {activeTab === 'ai' && (
                  <div className="space-y-4">
                    ...
                    {generationLoading && !currentGeneration && (
                      <div className="text-center py-8">
                        <p className="text-gray-500">Loading {generationType}...</p>
                      </div>
                    )}
                    
                    {!currentGeneration && !generationLoading && (
                      <div className="text-center py-8">
                        <p className="text-gray-500">No {generationType} generation available yet.</p>
                        <button
                    ...
                    {currentGeneration && (
                      <div>
                        ...
                        {currentGeneration.status === 'completed' ? (
                          ...
                          <div className="mt-4 bg-red-50 p-4 rounded-md">
                            <p className="text-red-700">Generation failed: {currentGeneration.error}</p>
                          </div>
                        ) : (
                          <div className="mt-4 text-center py-8">
                            <div className="flex flex-col items-center">
                              ...
                              <p className="mt-4 text-gray-500">Generation in progress...</p>
                            </div>
                          </div>
                        )}
                      </div>
...
const renderGenerationContent = (generation: any) => {
  switch (generation.type) {
    case 'summary':
      return (
        <div className="p-4 bg-gray-50 rounded-md">
          <p className="text-gray-700 whitespace-pre-line">{generation.summary}</p>
        </div>
      );
      ...
      return (
        <div className="p-4 bg-gray-50 rounded-md">
          <p className="text-gray-500 mb-4">
            Mind map visualization is not available in this view. 
            Below is a text representation of the mind map nodes:
          </p>
          {generation.mindmap && Object.values(generation.mindmap).map((node: any) => (
    ...
    default:
      return (
        <div className="p-4 bg-gray-50 rounded-md">
          <p className="text-gray-500">Content not available for this generation type.</p>
        </div>
      );
  }
};
