...
const StudyPage = () => {
  ...
  const { contents, isLoading: contentLoading, fetchContents } = useContentStore();
  const [activeTab, setActiveTab] = useState<'flashcards' | 'study-plan'>('flashcards');
  const [showLowConfidence, setShowLowConfidence] = useState(false);
  const [flashcardSets, setFlashcardSets] = useState<{ id: string; title: string; }[]>([
    ...
    <MainLayout>
      <div className="container mx-auto px-4 py-8">
        ...
        {activeTab === 'study-plan' && (
          <div>
            <StudyPlan 
              storageKeyPrefix="edusloth-study-personal"
            />
          </div>
        )}
      </div>
